...
// File Summary:
// The RevenuePage provides a full overview of vending machine revenue data,
// including a chart of revenue trends, month-by-month summaries, and machine-level details.
// It queries Supabase views (`monthly_revenue`, `monthly_revenue_by_month`), allows filtering
// by machine and year, and visualizes results in multiple formats.
//
// Key responsibilities:
// - Fetch aggregated revenue data (all machines, monthly totals, machine-level rows).
// - Allow users to filter by machine and year.
// - Compute available years dynamically from revenue data.
// - Render a chart of monthly totals, filling in missing months with zeros for continuity.
// - Provide two tabbed views: a month summary table and a machine-level revenue table.
// - Handle loading and error states gracefully.
//
// Dependencies:
// - supabase-js: queries revenue views (`monthly_revenue`, `monthly_revenue_by_month`).
// - Local components: RevenueChart, RevenueTable, MonthSummaryTable.
// - shadcn/ui: Tabs components for switching between views.
// - i18next: translation of titles and labels.
// - Local helpers: aggregateByMonth, fillMissingMonths from `src/lib/revenue-helpers`.
//
// Folder structure notes:
// - Located in `src/pages/`, this file represents the route `/revenue`.
// - Complements the detail page (`/revenue/:year/:month`) for drill-down analysis.
//
// Security notes:
// - Supabase RLS must permit:
//   • View `monthly_revenue`: SELECT for authenticated users (read-only).  
//   • View `monthly_revenue_by_month`: SELECT for authenticated users (read-only).  
// - Clients can only read aggregated values; inserts/updates are not permitted from this page.
...
export default function RevenuePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // Extract unique machines for machine filter
  ...
  // Derive list of available years for filter dropdown
  ...
  // Filter machine-level rows by selected year and machine
  ...
  // Prepare chart data: aggregate monthly totals, fill missing months
  const chartData = useMemo(() => {
    ...
    return fillMissingMonths(base, selectedYear).map(x => ({
      month: x.monthISO,
      total_revenue: x.total,
      currency: x.currency,
    }));
  }, [filteredRows, selectedMachine, selectedYear]);

  // Restrict monthly summary to selected year
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {/* Filters: machine and year dropdowns */}
      ...
      {/* Tabs for summary vs detail view */}
